...
DB: trading_system_v27.db (or v28.db when called from harness_v28)
"""
...
class MacroSupervisor:
    def __init__(self, db_path: Optional[str] = None, **kwargs):
        ...
        self.db_path  = db_path or os.path.join(_dir, "trading_system_v27.db")
    ...
    def _classify_regime5(self, paused, pause_trigger, drawdown,
        ...
        if bars_since_res is not None and bars_since_res < self.recovery_bars:
            return "RECOVERY"
        if (trend_strength > self.bull_ts_min
                and close > slow_ema
    ...
    def _compute_h1_signals(self, df1h: pd.DataFrame) -> pd.DataFrame:
        ...
        for idx in range(len(h)):
            ...
            if not paused:
                trigger_slow   = (dd < -self.pause_dd_trigger
                                  and rsi < self.pause_rsi_max
                                  and close_v < slow_ema * self.pause_ma_mult)
                trigger_rapid  = rapid_drop < -self.rapid_descent_trigger
...
def main():
    ap = argparse.ArgumentParser(description="MacroSupervisor v27 standalone")
